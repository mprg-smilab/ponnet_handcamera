...
class parception_branch(nn.Module):
    def __init__(self):
        ...
        #self.liner_meta = nn.Linear(4,256)

        self.rgb_balancing   = balancing(256,256)
        self.depth_balancing = balancing(256,256)

        self.parception1 = nn.Sequential(nn.BatchNorm1d(260),nn.Linear(260,256),nn.ReLU(inplace=True),
    ...
    def forward(self, input_rgb, input_depth, input_meta):
        ...
        #h_meta = self.liner_meta(input_meta)
        ##----------------Balancing------------
        ...
        input_list = [balanced_att, input_meta]
        input_concat = torch.cat(input_list,dim=1)

        y = self.parception1(input_concat)
        return y
...
if __name__ == "__main__":
    ...
    ponnet_dataset = dataloader.PonNetDataset(dataset_ver='Headcam_only')
